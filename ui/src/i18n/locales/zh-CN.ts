...
export const zh_CN: TranslationMap = {
  ...
  pipelines: {
    ...
    stepCount: "{count} 步",
    new: "+ 新建",
    createFirst: "点击「新建」创建第一条工作流。",
    pickPrompt: "从左侧选择一条工作流进行编辑。",
    ...
    agentMissing: "{id}（已失效）",
    instructionPlaceholder: "这一步做什么，例如：检索资料并保留出处",
    addStep: "+ 添加步骤",
    maxSteps: "最多 {max} 步",
    flowHint:
      "每步收到「原始请求 + 之前每一步的输出」，各份输出都标注了步骤号和 agent；某步失败则中断，已完成的产出仍会保留。",
    save: "保存",
    saving: "保存中…",
    delete: "删除",
    saveFailed: "保存失败",
    deleteFailed: "删除失败",
    errConflict: "工作流已被其他地方修改，列表已刷新为最新内容。",
    errNotFound: "该工作流不存在，可能已被删除。",
    unavailableTitle: "工作流功能不可用",
    unavailableBody:
      "需要启用 dragon-task-orchestrator 插件（在配置中设置 enabled: true）后重启网关。",
    invalid: {
      name_empty: "名称不能为空。",
      name_too_long: "名称不能超过 {max} 个字符。",
      no_steps: "至少需要一个步骤。",
      too_many_pipelines: "工作流数量不能超过 {max} 条。",
      too_many_steps: "步骤不能超过 {max} 个。",
      step_no_agent: "第 {step} 步未选择 agent。",
      step_internal_agent: "第 {step} 步不能使用内部 agent「{agentId}」。",
      step_unknown_agent: "第 {step} 步使用的 agent「{agentId}」不存在。",
      step_instruction_too_long: "第 {step} 步的指令超过 {max} 个字符。",
    },
  },
  chatMode: {
    trigger: "执行模式：{mode}",
  ...
  taskOrchestrator: {
    ...
    awaiting: "待确认",
    assigning: "分配中",
    verifyPassed: "校验 {attempts} 次通过",
    roleVerify: "校验",
    noCriteria: "无校验",
    criteriaLabel: "校验点",
    dependsOn: "← 依赖 {ids}",
    subtasksLabel: "子任务分配（按依赖层级）",
    unscheduled: "未调度（依赖无法满足）",
    layerN: "第 {layer} 层",
    layerOf: "第 {layer}/{total} 层",
    layerDone: "第 {layer}/{total} 层已完成",
    timelineLabel: "逐层进度",
    ...
    headerAllLayersDone: "{total} 层已全部完成",
    chipRedecomposing: "重新拆解中",
    summarizingText: "所有子任务已完成{note}，正在整合成最终回复，请稍候…",
    summarizingNote: "（{ok}/{total} 项成功）",
    redecomposingHead: "正在按你的意见重新拆解…",
    redecomposingHint: "通常需要 30-60 秒，完成后会展示新的方案",
    confirmQuestion: "是否按此方案执行？",
    confirmHint: "确认后才会开始执行；也可以填写调整意见，让拆解重新生成。",
    confirmRun: "确认执行",
    confirmCancel: "取消",
    adjustPlaceholder: "调整意见，例如：把第 2 和第 3 个子任务合并",
    adjustSubmit: "按调整意见重新拆解",
    decisionFailed: "提交失败",
    pipelineCardTitle: "工作流「{name}」执行进度",
    pipelineCardTitleUnnamed: "工作流执行进度",
    pipelineCount: "{count} 步 · {label}",
    pipelineStepsLabel: "工作流步骤（按顺序）",
    stepOf: "第 {step}/{total} 步",
    stepDone: "第 {step}/{total} 步已完成",
    allStepsDone: "{total} 步已全部完成",
    pipelineTimelineLabel: "逐步进度",
  },
  overview: {
    access: {
      title: "网关访问",
