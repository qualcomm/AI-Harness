...
export type ChatState = {
  ...
  /**
   * Guard session key set when a privacy_redirect event is received.
   * When set, loadChatHistory merges messages from both the main session
   * and the guard session (sorted by timestamp) so the user sees a unified
   * conversation view without the guard session messages being written to
...
/**
 * Merge two message arrays by timestamp (ascending).
 * Used to interleave main session and guard session messages into a unified view.
 */
function mergeMessagesByTimestamp(main: unknown[], guard: unknown[]): unknown[] {
  const all = [...main, ...guard];
  all.sort((a, b) => {
...
export async function loadChatHistory(state: ChatState) {
  ...
  try {
    ...
    // If a guard session is active and different from the current session, also load
    // its messages and merge by timestamp. Skip when the user has manually switched
    // to the guard session itself (sessionKey === guardSessionKey) to avoid duplicates.
    if (state.guardSessionKey && state.guardSessionKey !== state.sessionKey) {
      try {
        const guardRes = await state.client.request<{ messages?: Array<unknown> }>(
          "chat.history",
          { sessionKey: state.guardSessionKey, limit: 200 },
        );
        const guardMessages = Array.isArray(guardRes.messages) ? guardRes.messages : [];
        if (guardMessages.length > 0) {
          messages = mergeMessagesByTimestamp(messages, guardMessages);
        }
      } catch {
        // Guard session may not exist yet — ignore silently
      }
    }
...
export function handleChatEvent(state: ChatState, payload?: ChatEventPayload) {
  ...
  const isMainSession = payload.sessionKey === state.sessionKey;
  // Also accept streaming deltas from the guard session so the user sees
  // the local model's response in real-time while it is being generated.
  const isGuardSession = Boolean(
    state.guardSessionKey && payload.sessionKey === state.guardSessionKey,
  );

  if (!isMainSession && !isGuardSession) {
    return null;
  }

  // For guard session events, only forward streaming deltas to chatStream.
  // Final/aborted/error are handled by the main session's broadcastChatFinal
  // which triggers loadChatHistory (merging guard session messages).
  if (isGuardSession && !isMainSession) {
