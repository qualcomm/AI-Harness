...
/**
 * `session.message` is broadcast for every persisted message, our own included, and
 * `loadChatHistory` replaces `chatMessages` wholesale while clearing the stream. Reloading
 * on those broadcasts mid-run wipes the optimistic echo of the user's own message and the
 * live stream, which is why a sent message appeared to vanish and the whole turn arrived
 * late. Deferred, not dropped — a message from elsewhere must still land.
 */
describe("session.message history reloads", () => {
  beforeEach(() => {
    gatewayClientInstances.length = 0;
    loadChatHistoryMock.mockClear();
  });

  function sessionMessage(client: GatewayClientMock, sessionKey = "main") {
    client.emitEvent({ event: "session.message", payload: { sessionKey } });
  }

  function finalChat(client: GatewayClientMock, runId = "engine-run-1") {
    client.emitEvent({
      event: "chat",
      payload: {
        runId,
  ...
  it("reloads immediately when no run of ours is in flight", () => {
    const { client } = connectHostGateway();
    sessionMessage(client);
    expect(loadChatHistoryMock).toHaveBeenCalledTimes(1);
  });

  // The reported defect: this reload is what erased the just-sent message.
  it("does not reload while our own run is streaming", () => {
    const { host, client } = connectHostGateway();
    host.chatRunId = "engine-run-1";
    sessionMessage(client);
    expect(loadChatHistoryMock).not.toHaveBeenCalled();
    expect(host.pendingHistoryReload).toBe(true);
  });

  it("still ignores messages for a session we are not viewing", () => {
    const { host, client } = connectHostGateway();
    sessionMessage(client, "other");
    expect(loadChatHistoryMock).not.toHaveBeenCalled();
    expect(host.pendingHistoryReload).toBe(false);
  });

  // Deferral must not become loss: the final event reconciles exactly once.
  it("reloads once at the end of the run, not once per deferred broadcast", () => {
    const { host, client } = connectHostGateway();
    host.chatRunId = "engine-run-1";
    sessionMessage(client);
    sessionMessage(client);
  ...
  // Tool output already forces its own reload; the deferral must not add a second one.
  it("does not double-reload when tool results already triggered one", () => {
    const { host, client } = connectHostGateway();
    host.chatRunId = "engine-run-1";
    emitToolResultEvent(client);
    sessionMessage(client);
  ...
  // `aborted` and `error` clear `chatRunId` too, so they end the deferral as well —
  // otherwise a stopped run would leave the flag set and the message unshown until the
  // next unrelated broadcast.
  for (const state of ["aborted", "error"] as const) {
    it(`applies a deferred reload after a run ends in ${state}`, () => {
      const { host, client } = connectHostGateway();
      host.chatRunId = "engine-run-1";
      sessionMessage(client);
      expect(loadChatHistoryMock).not.toHaveBeenCalled();
      ...
      client.emitEvent({
        event: "chat",
        payload: { runId: "engine-run-1", sessionKey: "main", state },
      });
      expect(loadChatHistoryMock).toHaveBeenCalledTimes(1);
      expect(host.pendingHistoryReload).toBe(false);
    });
  ...
  it("reloads normally again once the run has finished", () => {
    const { host, client } = connectHostGateway();
    host.chatRunId = "engine-run-1";
    sessionMessage(client);
    finalChat(client);
