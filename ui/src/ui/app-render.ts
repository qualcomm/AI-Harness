      ...
      <main class="content ${isChat ? "content--chat" : ""}">
        ...
        ${state.tab === "chat"
          ? renderChat({
              ...
              taskOrchestratorProgress: (state as unknown as OpenClawApp)
                .taskOrchestratorProgress,
              taskOrchestratorBusy: (state as unknown as OpenClawApp).taskOrchestratorBusy,
              taskOrchestratorError: (state as unknown as OpenClawApp).taskOrchestratorError,
              onTaskOrchestratorDecision: (
                approvalId: string,
                decision: "confirm" | "cancel" | "adjust",
                adjustment?: string,
              ) =>
                void (state as unknown as OpenClawApp).handleTaskOrchestratorPrdDecision(
                  approvalId,
                  decision,
                  adjustment,
                ),
              sessionMode: (state as unknown as OpenClawApp).sessionMode,
              sessionModeBusy: (state as unknown as OpenClawApp).sessionModeBusy,
              sessionModeMenuOpen: (state as unknown as OpenClawApp).sessionModeMenuOpen,
              sessionModePipelines: (
                state as unknown as OpenClawApp
              ).pipelinesState.pipelines.map((p) => ({ id: p.id, name: p.name })),
              onSessionModeMenuToggle: (open: boolean) => {
                (state as unknown as OpenClawApp).sessionModeMenuOpen = open;
              },
              onSessionModeSelect: (mode) =>
                void (state as unknown as OpenClawApp).handleSessionModeSelect(mode),
              // Connects the use surface to the edit surface: without this the operator
              // has to find the page themselves.
              onManagePipelines: () => state.setTab("pipelines"),
        ...
        ${state.tab === "pipelines"
          ? renderPipelines({
              ...(state as unknown as OpenClawApp).pipelinesState,
              // Internal orchestrator identities are excluded: they carry
              // `tools.deny: ["*"]`, so a step routed to one could never do real work.
              agentIds: (state.agentsList?.agents ?? [])
                .map((a) => a.id)
                .filter((id) => !id.startsWith("dt-")),
              draggingIndex: (state as unknown as OpenClawApp).pipelineDragIndex,
              dropTargetIndex: (state as unknown as OpenClawApp).pipelineDropIndex,
              onSelect: (id) => (state as unknown as OpenClawApp).handlePipelineSelect(id),
              onNew: () => (state as unknown as OpenClawApp).handlePipelineNew(),
              onSave: () => void (state as unknown as OpenClawApp).handlePipelineSave(),
              onDelete: (id) => void (state as unknown as OpenClawApp).handlePipelineDelete(id),
              onDraftChange: (mutate) =>
                (state as unknown as OpenClawApp).handlePipelineDraftChange(mutate),
              onDragState: (dragging, dropTarget) =>
                (state as unknown as OpenClawApp).handlePipelineDragState(dragging, dropTarget),
            })
          : nothing}
        ${state.tab === "dreams"
          ? renderDreaming({
