...
import { loadPipelines } from "./controllers/pipelines.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  switch (host.tab) {
    ...
    case "dreams":
      ...
      return;
    case "pipelines":
      // Agents are needed for the step dropdowns, pipelines for the list itself.
      await Promise.all([
        loadAgents(app),
        loadPipelines(host as unknown as Parameters<typeof loadPipelines>[0]),
      ]);
      return;
    case "chat":
