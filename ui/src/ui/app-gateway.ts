...
} from "./chat/task-orchestrator-events.ts";
...
type GatewayHost = {
  ...
  /**
   * Set when a `session.message` reload was skipped because our own run was streaming.
   * Consumed at the end of that run so a message from another source is never lost.
   */
  ...
  taskOrchestratorProgress: Map<string, TaskOrchestratorProgress>;
  /**
   * Guard session key received via privacy_redirect event.
   * When set, loadChatHistory merges guard session messages into the main view.
   */
  guardSessionKey: string | null;
};
...
function handleChatGatewayEvent(host: GatewayHost, payload: ChatEventPayload | undefined) {
  ...
  if (state === "final" && !historyReloaded && shouldReloadHistoryForFinalEvent(payload)) {
    host.pendingHistoryReload = false;
    void loadChatHistory(host as unknown as ChatState);
    return;
  }
  // A `session.message` deferred during this run still has to be applied. Only the
  // terminal states clear `chatRunId`, so this is the point where the deferral ends —
  // and `historyReloaded` already covers the case, so it is not requested twice.
  if (isTerminalChatState(state) && host.pendingHistoryReload) {
    host.pendingHistoryReload = false;
    if (!historyReloaded) {
      void loadChatHistory(host as unknown as ChatState);
    }
...
/**
 * A message was persisted on a session we are viewing.
 *
 * The gateway broadcasts this for EVERY persisted message, including the user message we
 * just sent and each of our own tool results. `loadChatHistory` replaces `chatMessages`
 * wholesale and clears `chatStream`/tool-stream state, so reloading on those broadcasts
 * destroys exactly the state that is making the turn visible:
 *
 * - the optimistic echo of the user's own message (controllers/chat.ts) vanishes until
 *   the reload lands, which is the "my message disappears then comes back" report;
 * - the in-flight stream and tool cards are cleared and only reappear on the next reload.
 *
 * Across a WAN-latency gateway that window is plainly visible, and one turn triggers many
 * of these broadcasts, so the reloads cancel each other (`shouldApplyChatHistoryResult`)
 * and the whole turn appears to arrive at once, late.
 *
 * While OUR run is streaming, the reload is also redundant: `handleChatEvent` renders the
 * assistant text and tool activity live, and `handleTerminalChatEvent` already reloads at
 * the end to swap streaming artifacts for the persisted tool results. So it is deferred
 * rather than dropped — a message from another client or channel arriving mid-run must
 * still show up, which `pendingHistoryReload` guarantees at the end of the run.
 */
function handleSessionMessageGatewayEvent(
  ...
  if (host.chatRunId) {
    host.pendingHistoryReload = true;
    return;
  }
  void loadChatHistory(host as unknown as ChatState);
}
...
function handleGatewayEventUnsafe(host: GatewayHost, evt: GatewayEventFrame) {
  ...
  if (evt.event === "privacy_redirect") {
    const payload = evt.payload as
      | { originalSessionKey?: string; guardSessionKey?: string }
      | undefined;
    if (payload?.guardSessionKey && payload.originalSessionKey === host.sessionKey) {
      console.log(
        `[privacy] WebUI received privacy_redirect: guard session = ${payload.guardSessionKey}`,
      );
      host.guardSessionKey = payload.guardSessionKey;
    }
    return;
  }
  ...
  if (evt.event === "plugin_event") {
    ...
    // dragon-task-orchestrator: PRD + per-layer progress. Unlike the banner above
    // this accumulates per rootSessionKey — see chat/task-orchestrator-events.ts.
    const nextProgress = applyTaskOrchestratorEvent(
      (host as unknown as { taskOrchestratorProgress: Map<string, TaskOrchestratorProgress> })
        .taskOrchestratorProgress ?? new Map(),
      payload,
      Date.now(),
    );
    if (nextProgress) {
      (
        host as unknown as { taskOrchestratorProgress: Map<string, TaskOrchestratorProgress> }
      ).taskOrchestratorProgress = nextProgress;
    }
    return;
  }
