...
import type { ChatSessionMode } from "./chat/mode-selector.ts";
...
export class OpenClawApp extends LitElement {
  ...
  /**
   * dragon-task-orchestrator PRD + per-layer progress, keyed by rootSessionKey.
   *
   * Accumulating (a Map of records that grow) rather than a single overwritten
   * value like `privacyStatus`: progress arrives as one event per layer, so
   * replacing on each event would erase every earlier layer. `plugin_event` is an
   * undifferentiated broadcast, so entries for other sessions land here too and
   * the chat view filters by its own session key.
   */
  @state() taskOrchestratorProgress: Map<
    string,
    import("./chat/task-orchestrator-card.ts").TaskOrchestratorProgress
  > = new Map();
  /** In-flight guard + last error for answering a PRD confirmation. */
  @state() taskOrchestratorBusy = false;
  @state() taskOrchestratorError: string | null = null;
  /**
   * Orchestration mode for the CURRENT session, mirrored from the plugin.
   *
   * Held here rather than derived because the plugin owns the state (per session,
   * process-local, default `off`) and a gateway restart clears it — so this is
   * refreshed from `dragonTaskOrchestrator.session.mode.get` on every session change
   * rather than assumed to persist.
   *
   * `null` means "not known yet / plugin not available", which is what hides the
   * selector: a plugin that is not installed must not grow a toolbar control.
   */
  @state() sessionMode: ChatSessionMode | null = null;
  @state() sessionModeBusy = false;
  @state() sessionModeMenuOpen = false;
  /** Fixed-pipeline list + editor state; see controllers/pipelines.ts. */
  @state() pipelinesState: PipelinesState = initialPipelinesState();
  /** Drag state for the pipeline editor, owned here so a re-render does not drop it. */
  @state() pipelineDragIndex: number | null = null;
  @state() pipelineDropIndex: number | null = null;
  @state() guardSessionKey: string | null = null;
  ...
  // Not `@state()`: it gates a fetch rather than anything rendered, so a change to it
  // must not trigger a re-render. See handleSessionMessageGatewayEvent.
  ...
  protected updated(changed: Map<PropertyKey, unknown>) {
    ...
    // Mission mode is per session and lives in the plugin's process memory, so it has
    // to be re-read whenever the session changes or the connection comes back — the
    // previous session's value says nothing about this one, and a reconnect may be
    // talking to a gateway that restarted and cleared the store.
    if (changed.has("sessionKey") || changed.has("connected")) {
      void this.refreshSessionMode();
    }
  ...
  /**
   * Answer a dragon-task-orchestrator PRD confirmation.
   *
   * Mirrors handleExecApprovalDecision above, but targets the plugin's own gateway
   * method: the host approval flow cannot carry a decomposition or an adjustment
   * request (see the plugin's index.ts).
   *
   * The awaiting state is not cleared here — the plugin broadcasts the next PRD or
   * the first layer's progress, and that event is what settles the card. Clearing
   * locally would hide the controls before knowing the orchestrator accepted.
   */
  async handleTaskOrchestratorPrdDecision(
    approvalId: string,
    decision: "confirm" | "cancel" | "adjust",
    adjustment?: string,
  ) {
    if (!this.client || this.taskOrchestratorBusy) {
      return;
    }
    this.taskOrchestratorBusy = true;
    this.taskOrchestratorError = null;
    try {
      await this.client.request("dragonTaskOrchestrator.prd.resolve", {
        approvalId,
        decision,
        ...(adjustment ? { adjustment } : {}),
      });
    } catch (err) {
      this.taskOrchestratorError = `${t("taskOrchestrator.decisionFailed")}: ${String(err)}`;
    } finally {
      this.taskOrchestratorBusy = false;
    }
  }

  /**
   * Read the session's orchestration mode back from the plugin, plus the pipeline list
   * the selector offers.
   *
   * A failure sets `sessionMode` to null, which hides the selector. That is the right
   * response to every failure here: the method is missing (plugin not installed), the
   * plugin is disabled in config, or the gateway is unreachable — in all three cases
   * there is nothing to switch.
   */
  async refreshSessionMode() {
    if (!this.client || !this.sessionKey) {
      this.sessionMode = null;
      return;
    }
    try {
      const result = (await this.client.request("dragonTaskOrchestrator.session.mode.get", {
        sessionKey: this.sessionKey,
      })) as { mode?: ChatSessionMode } | undefined;
      this.sessionMode = result?.mode ?? { kind: "off" };
      // Loaded together so the dropdown can name the selected pipeline instead of
      // showing an id.
      await loadPipelines(this as unknown as PipelinesHost);
    } catch {
      this.sessionMode = null;
    }
  }
   ...
   *
   * The new state comes from the gateway's echo rather than from optimistically setting
   * it locally, so the selector can never show a mode the plugin does not hold — that
   * mismatch would make the next request behave contrary to the UI.
   */
  async handleSessionModeSelect(mode: ChatSessionMode) {
    this.sessionModeMenuOpen = false;
    if (!this.client || !this.sessionKey || this.sessionModeBusy || this.sessionMode === null) {
      return;
    }
    this.sessionModeBusy = true;
    try {
      const result = (await this.client.request("dragonTaskOrchestrator.session.mode.set", {
        sessionKey: this.sessionKey,
        mode,
      })) as { mode?: ChatSessionMode } | undefined;
      this.sessionMode = result?.mode ?? this.sessionMode;
    } catch {
      // Re-read instead of guessing: a failed write leaves the gateway authoritative.
      await this.refreshSessionMode();
    } finally {
      this.sessionModeBusy = false;
    }
  }

  // ── Pipeline editor ───────────────────────────────────────────────
  //
  // The controller replaces `pipelinesState` wholesale on every mutation, which is what
  // triggers Lit's reference-based reactivity — no manual sync needed here.

  handlePipelineSelect(id: string | null) {
    selectPipeline(this as unknown as PipelinesHost, id);
  }

  handlePipelineNew() {
    startNewPipeline(this as unknown as PipelinesHost);
  }

  handlePipelineDraftChange(mutate: (draft: PipelineDraft) => PipelineDraft) {
    updateDraft(this as unknown as PipelinesHost, mutate);
  }

  async handlePipelineSave() {
    await savePipeline(this as unknown as PipelinesHost);
  }

  async handlePipelineDelete(id: string) {
    await removePipeline(this as unknown as PipelinesHost, id);
    // A deleted pipeline may have been this session's selected mode; the plugin clears
    // that server-side, so re-read rather than leaving the selector showing it.
    await this.refreshSessionMode();
  }

  handlePipelineDragState(dragging: number | null, dropTarget: number | null) {
    this.pipelineDragIndex = dragging;
    this.pipelineDropIndex = dropTarget;
  }
