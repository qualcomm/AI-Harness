...
import type { TaskOrchestratorProgress } from "../chat/task-orchestrator-events.ts";
...
describe("chat view", () => {
  ...
  describe("mode selector", () => {
    function drawWith(overrides: Partial<ChatProps>) {
      const container = document.createElement("div");
      render(renderChat(createProps(overrides)), container);
      return container;
    }

    // A dead control is worse than no control: null means the orchestrator plugin is
    // absent or disabled in config, so there is nothing to switch.
    it("is not rendered when the orchestrator is unavailable", () => {
      expect(drawWith({ sessionMode: null }).querySelector(".chat-mode__trigger")).toBeNull();
      expect(drawWith({}).querySelector(".chat-mode__trigger")).toBeNull();
    });

    // With three-plus states colour alone cannot say which is active, so the trigger
    // names the current mode.
    it("names the current mode on the trigger", () => {
      expect(
        drawWith({ sessionMode: { kind: "off" } }).querySelector(".chat-mode__current")!
          .textContent,
      ).toContain("Plain chat");
      expect(
        drawWith({ sessionMode: { kind: "dynamic" } }).querySelector(".chat-mode__current")!
          .textContent,
      ).toContain("Dynamic breakdown");
      expect(
        drawWith({
          sessionMode: { kind: "pipeline", pipelineId: "pl_a" },
          sessionModePipelines: [{ id: "pl_a", name: "文档生成" }],
        }).querySelector(".chat-mode__current")!.textContent,
      ).toContain("文档生成");
    });

    // The session still points at it and the plugin passes turns through until changed,
    // so saying so beats showing a blank or a raw id.
    it("says so when the selected pipeline no longer exists", () => {
      const container = drawWith({
        sessionMode: { kind: "pipeline", pipelineId: "pl_gone" },
        sessionModePipelines: [],
      });
      expect(container.querySelector(".chat-mode__current")!.textContent).toContain("(missing)");
    });

    it("marks off as inactive and the other modes as active", () => {
      const off = drawWith({ sessionMode: { kind: "off" } }).querySelector(".chat-mode__trigger")!;
      expect(off.classList.contains("chat-mode__trigger--active")).toBe(false);
      const on = drawWith({ sessionMode: { kind: "dynamic" } }).querySelector(
        ".chat-mode__trigger",
      )!;
      expect(on.classList.contains("chat-mode__trigger--active")).toBe(true);
    });

    it("shows the menu only when open, and lists every pipeline", () => {
      expect(
        drawWith({ sessionMode: { kind: "off" } }).querySelector(".chat-mode__menu"),
      ).toBeNull();
      const container = drawWith({
        sessionMode: { kind: "off" },
      ...
      const text = container.querySelector(".chat-mode__menu")!.textContent ?? "";
      expect(text).toContain("Plain chat");
    ...
    it("reports the chosen mode", () => {
      const onSessionModeSelect = vi.fn();
      const container = drawWith({
        sessionMode: { kind: "off" },
        sessionModeMenuOpen: true,
        sessionModePipelines: [{ id: "pl_a", name: "文档生成" }],
      ...
      const options = container.querySelectorAll<HTMLButtonElement>(".chat-mode__option");
      // off, dynamic, pipeline, manage
      options[1]!.dispatchEvent(new MouseEvent("click", { bubbles: true }));
      expect(onSessionModeSelect).toHaveBeenCalledWith({ kind: "dynamic" });
      options[2]!.dispatchEvent(new MouseEvent("click", { bubbles: true }));
      expect(onSessionModeSelect).toHaveBeenCalledWith({
        kind: "pipeline",
        pipelineId: "pl_a",
      });
    });

    it("says so when no pipelines are defined yet", () => {
      const container = drawWith({
        sessionMode: { kind: "off" },
        sessionModeMenuOpen: true,
        sessionModePipelines: [],
    ...
    it("offers a way to reach the pipelines page", () => {
      const onManagePipelines = vi.fn();
      const container = drawWith({
        sessionMode: { kind: "off" },
        sessionModeMenuOpen: true,
        onManagePipelines,
      ...
      container
        .querySelector<HTMLButtonElement>(".chat-mode__option--manage")!
        .dispatchEvent(new MouseEvent("click", { bubbles: true }));
      expect(onManagePipelines).toHaveBeenCalledTimes(1);
    });

    it("is disabled while a switch is in flight, so a double click cannot race", () => {
      const container = drawWith({ sessionMode: { kind: "off" }, sessionModeBusy: true });
      expect(
        container.querySelector<HTMLButtonElement>(".chat-mode__trigger")!.disabled,
      ).toBe(true);
    });
  });
...
/**
 * Where the orchestrator card sits in the flow.
 *
 * The card is a single floating element with no owning message, so its position is a
 * computed rule — and the rule regressed once already: keyed off the TRAILING item, the
 * card followed whichever turn was newest, so a plain-chat message sent after a pipeline
 * run dragged the pipeline's card down to it. These tests pin the rule to the plan's own
 * turn instead.
 */
describe("task orchestrator card placement", () => {
  const PLAN_AT = 1000;

  function progressAt(startedAt: number): Map<string, TaskOrchestratorProgress> {
    return new Map([
      [
        "main",
        {
  ...
  function msg(id: string, role: string, timestamp: number, text: string) {
    return normalizeMessage({ id, role, content: [{ type: "text", text }], timestamp });
  }

  /** Rendered order of the card against the assistant replies around it. */
  function layout(overrides: Partial<ChatProps>): string[] {
    const container = document.createElement("div");
    render(renderChat(createProps(overrides)), container);
    const out: string[] = [];
    // Document order: the card is emitted as a sibling immediately before its anchor
    // group, so a flat query is enough to read off the resulting sequence.
    for (const el of container.querySelectorAll(".dto-card, .chat-group")) {
      if (el.classList.contains("dto-card")) {
        out.push("CARD");
        continue;
      }
      const text = el.textContent ?? "";
      const marker = ["pipeline-reply", "plain-reply", "second-question"].find((m) =>
        text.includes(m),
      );
  ...
  // Nothing from this turn has landed yet, so there is no anchor and the tail is right.
  it("sits at the tail while the run is still executing", () => {
    const order = layout({
      taskOrchestratorProgress: progressAt(PLAN_AT),
      messages: [msg("u1", "user", PLAN_AT - 10, "do the thing")],
    });
    expect(order.at(-1)).toBe("CARD");
  });

  it("moves above the reply its own turn produced", () => {
    const order = layout({
      taskOrchestratorProgress: progressAt(PLAN_AT),
      messages: [
        msg("u1", "user", PLAN_AT - 10, "do the thing"),
        msg("a1", "assistant", PLAN_AT + 50, "pipeline-reply"),
      ],
    });
    expect(order).toEqual(["CARD", "pipeline-reply"]);
  });

  // The regression this rule exists for: a later turn must not pull the card along.
  it("stays with its own reply after a plain-chat turn follows", () => {
    const order = layout({
      taskOrchestratorProgress: progressAt(PLAN_AT),
      messages: [
        msg("u1", "user", PLAN_AT - 10, "do the thing"),
        msg("a1", "assistant", PLAN_AT + 50, "pipeline-reply"),
        msg("u2", "user", PLAN_AT + 100, "second-question"),
        msg("a2", "assistant", PLAN_AT + 150, "plain-reply"),
      ],
    });
    expect(order).toEqual(["CARD", "pipeline-reply", "second-question", "plain-reply"]);
  });

  // The intermediate state that produced the reported symptom: with only the new user
  // message present, a trailing-item rule found no anchor and dumped the card at the end.
  it("does not fall to the tail while the next turn is mid-flight", () => {
    const order = layout({
      taskOrchestratorProgress: progressAt(PLAN_AT),
      messages: [
        msg("u1", "user", PLAN_AT - 10, "do the thing"),
    ...
    expect(order).toEqual(["CARD", "pipeline-reply", "second-question"]);
  });

  /**
   * The reported layout, exactly: the plain turn's tool cards ended up ABOVE the pipeline
   * card and its reply BELOW it, so the card appeared wedged inside the later turn.
   *
   * Under the old trailing-item rule the card was placed before the LAST index — the plain
   * reply — which put everything else in that turn, tool cards included, above it. Tool
   * groups normalize to role "tool" rather than "assistant", so they were never candidate
   * anchors themselves; they just happened to sit between.
   */
  it("stays above its own reply when the next turn has tool cards", () => {
    const toolMsg = normalizeMessage({
      id: "t1",
      role: "toolResult",
      toolName: "bash",
      content: [{ type: "text", text: "exit 0" }],
      timestamp: PLAN_AT + 120,
    });
    const container = document.createElement("div");
    render(
      renderChat(
    ...
    for (const el of container.querySelectorAll(".dto-card, .chat-group")) {
      ...
      // Identified by class, not text: a tool group renders collapsed by default, so its
      // body — and any marker inside it — is not in the DOM at all.
      if (el.classList.contains("tool")) {
        order.push("tool-card");
        continue;
      }
      const text = el.textContent ?? "";
      const marker = ["pipeline-reply", "second-question", "plain-reply"].find((m) =>
        text.includes(m),
      );
      if (marker) order.push(marker);
    }
    // The card leads its own turn; the entire later turn follows it, in its own order.
  ...
  // The other direction, which the timestamp comparison guards: a plan that has only just
  // arrived must not attach itself above a reply from a previous turn.
  it("ignores replies older than the plan", () => {
    const order = layout({
      taskOrchestratorProgress: progressAt(PLAN_AT),
      messages: [msg("a0", "assistant", PLAN_AT - 500, "pipeline-reply")],
    });
    expect(order).toEqual(["pipeline-reply", "CARD"]);
  });

  it("renders nothing when this session has no plan", () => {
    const container = document.createElement("div");
    render(
      renderChat(
        createProps({ messages: [msg("a1", "assistant", PLAN_AT, "plain-reply")] }),
      ),
      container,
    );
    expect(container.querySelector(".dto-card")).toBeNull();
  });
});
