...
} from "../chat/task-orchestrator-card.ts";
import { renderModeSelector, type ChatSessionMode } from "../chat/mode-selector.ts";
import { findProgressForSession } from "../chat/task-orchestrator-events.ts";
...
export type ChatProps = {
  ...
  /**
   * dragon-task-orchestrator progress keyed by rootSessionKey. The card only
   * renders the entry matching this view's own `sessionKey` — `plugin_event` is a
   * broadcast, so entries for other sessions are present too.
   */
  taskOrchestratorProgress?: Map<string, TaskOrchestratorProgress> | null;
  /**
   * Answer the PRD confirmation gate. Absent when the host has not wired it, in
   * which case the card still renders read-only.
   */
  onTaskOrchestratorDecision?: (
    approvalId: string,
    decision: TaskOrchestratorDecision,
    adjustment?: string,
  ) => void;
  taskOrchestratorBusy?: boolean;
  taskOrchestratorError?: string | null;
  /**
   * Orchestration mode for this session, or null when the orchestrator plugin is
   * unavailable — in which case the selector is not rendered at all rather than
   * rendered disabled, since there is nothing to switch.
   */
  sessionMode?: ChatSessionMode | null;
  sessionModeBusy?: boolean;
  /** Pipelines offered in the dropdown; empty is a valid state (none defined yet). */
  sessionModePipelines?: Array<{ id: string; name: string }>;
  sessionModeMenuOpen?: boolean;
  onSessionModeMenuToggle?: (open: boolean) => void;
  onSessionModeSelect?: (mode: ChatSessionMode) => void;
...
/**
 * Collapse-state key for the orchestrator card. It shares the per-session tool
 * card map, so the id must not collide with the `toolcard:`/`tool:` ids built
 * from message content.
 */
const TASK_ORCHESTRATOR_CARD_ID = "dragon-task-orchestrator:progress";
...
export function renderChat(props: ChatProps) {
  ...
  const taskOrchestratorProgress = findProgressForSession(
    props.taskOrchestratorProgress,
    props.sessionKey,
  );

  /**
   * Where to place the orchestrator card: immediately before the reply THIS plan
   * produced, so the plan reads above the answer it explains.
   *
   * The anchor is the FIRST assistant group no older than the plan, not the trailing
   * item. That distinction is the whole point:
   *
   * - A trailing-item rule ("is the last thing an assistant group?") makes the card
   *   follow whatever turn is newest. Send a plain-chat message after a pipeline run and
   *   the card leaves its own turn: while the new message or its stream is trailing it
   *   has no anchor at all and falls to the tail (below the new content), and once the
   *   new reply lands it re-attaches above THAT reply — a turn it has nothing to do with.
   * - The first matching group is fixed for the life of the plan. Later turns append
   *   groups after it and cannot change which one matches, so the card stays put.
   *
   * `timestamp >= startedAt` still does the cross-turn work it was added for — it stops
   * the card attaching to a reply from an EARLIER turn — but "first" rather than "last"
   * is what stops it drifting into a later one.
   *
   * Null means no reply from this turn exists yet (awaiting confirmation, or executing),
   * where the card belongs at the tail; see the fallback after the loop.
   *
   * `groupMessages` folds every message into a group, so only `kind: "group"` needs
   * checking here.
   */
  const planStartedAt = taskOrchestratorProgress?.startedAt ?? null;
  const anchorIndex =
    planStartedAt === null
      ? -1
      : chatItems.findIndex(
          (item) =>
            ...
            item.timestamp >= planStartedAt,
        );
  const cardBeforeIndex = anchorIndex === -1 ? null : anchorIndex;

  /**
   * True once THIS plan's reply is in the flow — the card needs it to stop the
   * "summarizing" spinner, since the summary IS the reply and its arrival is the only
   * completion signal the plugin sends.
   *
   * Derived from the anchor rather than computed separately, so the placement rule and
   * the spinner's stop condition cannot drift apart. Being anchor-based also makes it
   * monotonic: the previous trailing-item version flipped back to false as soon as the
   * user typed again, which re-showed the spinner over a finished plan.
   */
  const orchestratorReplyLanded = cardBeforeIndex !== null;

  // Built once so it can be placed in either of two positions without duplicating
  // the wiring below.
  const taskOrchestratorCard = renderTaskOrchestratorCard(taskOrchestratorProgress, {
    expanded: expandedToolCards.get(TASK_ORCHESTRATOR_CARD_ID) ?? true,
    onToggleExpanded: () => {
      expandedToolCards.set(
        TASK_ORCHESTRATOR_CARD_ID,
        !(expandedToolCards.get(TASK_ORCHESTRATOR_CARD_ID) ?? true),
      );
      requestUpdate();
    },
    // The card tracks which decision was made; the approvalId to answer with lives
    // on the progress entry, so it is supplied here.
    onDecision: (decision, adjustment) => {
      if (!taskOrchestratorProgress?.approvalId) return;
      props.onTaskOrchestratorDecision?.(
        taskOrchestratorProgress.approvalId,
        decision,
        adjustment,
      );
    },
    busy: props.taskOrchestratorBusy,
    error: props.taskOrchestratorError,
    replyLanded: orchestratorReplyLanded,
  });
    ...
    >
      <div class="chat-thread-inner">
        ...
        ${repeat(
          ...
          (item) => item.key,
          (item, index) => {
            const cardHere = index === cardBeforeIndex ? taskOrchestratorCard : nothing;
            ...
            if (item.kind === "group") {
              if (deleted.has(item.key)) {
                // Still emit the card: the anchor group being deleted must not take
                // the plan display down with it.
                return cardHere;
              }
              return html`${cardHere}${renderMessageGroup(item, {
        ...
        ${
          // Tail placement is the fallback: while awaiting confirmation or executing this
          // turn has produced no assistant group yet, so there is nothing to anchor to and
          // the card belongs last. Once its reply lands that group becomes the anchor and
          // the card moves above it, permanently (see cardBeforeIndex).
          cardBeforeIndex === null ? taskOrchestratorCard : nothing
        }
      </div>
    </div>
  `;
          ...
          <div class="agent-chat__toolbar-right">
            ...
            ${canAbort
              ...
              : html`
                ...
                `}
            ${renderModeSelector({
              mode: props.sessionMode ?? null,
              pipelines: props.sessionModePipelines ?? [],
              busy: props.sessionModeBusy === true,
              open: props.sessionModeMenuOpen === true,
              onToggleOpen: (open) => props.onSessionModeMenuToggle?.(open),
              onSelect: (mode) => props.onSessionModeSelect?.(mode),
              onManagePipelines: () => props.onManagePipelines?.(),
            })}
            <button
              class="btn btn--ghost"
