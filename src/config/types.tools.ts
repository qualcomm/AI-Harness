...
export type ToolsConfig = {
  ...
  web?: {
    search?: {
      ...
      maxResults?: number;
      /**
       * Cap on the serialized size of one `web_search` tool result, in characters.
       *
       * Providers return snippets of whatever length they like — a single observed
       * Tavily result was 29,730 characters — and a research agent easily makes 20+
       * searches in one turn. Uncapped, that overflows the model's context mid-loop and
       * forces auto-compaction, which in one measured run cost 48s plus a wasted tool
       * loop and a failed verification.
       *
       * Applies to the managed `web_search` tool for every provider. Provider-specific
       * tools (for example `tavily_search`) are not affected — those are opted into
       * explicitly for their extra options.
       *
       * Default: 12000. Set 0 to disable capping.
       */
