...
export const chatHandlers: GatewayRequestHandlers = {
  "chat.history": async ({ params, respond, context }) => {
    ...
    // GuardClaw display session: if a <sessionId>.display.jsonl file exists alongside
    // the main session file, serve it to the WebUI so users see real (non-desensitized)
    // content while the cloud model continues to read the desensitized main session.
    let localMessages: unknown[];
    if (sessionId && storePath) {
      // `storePath` identifies the sessions DIRECTORY, so it belongs in the
      // path-options argument — the second parameter is the store entry, not a path.
      const mainFile =
        entry?.sessionFile ??
        resolveSessionFilePath(sessionId, undefined, {
          sessionsDir: path.dirname(storePath),
        });
      const displayFile = mainFile ? mainFile.replace(/\.jsonl$/, ".display.jsonl") : null;
      if (displayFile && fs.existsSync(displayFile)) {
        // Read display session (real content) for WebUI rendering
        ...
        localMessages = readSessionMessages(sessionId, storePath, entry?.sessionFile);
      }
    } else {
      localMessages = [];
    }
    ...
    if (hookRunner?.hasHooks("resolve_model")) {
      try {
        ...
        if (hookResult?.sessionKey && hookResult.sessionKey !== p.sessionKey) {
          ...
          // Notify WebUI so it can subscribe to guard session streaming and load history from it
          context.broadcast("privacy_redirect", {
            originalSessionKey: p.sessionKey,
            guardSessionKey: effectiveSessionKey,
          });
        }
    ...
    try {
      ...
      void dispatchInboundMessage({
        ...
        .then(async () => {
          ...
          // If using guard session isolation, close the original run on the main session.
          // The WebUI received a privacy_redirect event earlier and will call loadChatHistory
          // against the guard session JSONL directly — no writes to main session JSONL needed.
          if (originalSessionKey && originalSessionKey !== effectiveSessionKey) {
            context.logGateway.info(
              `[privacy] Guard session completed for ${originalSessionKey}. Closing run ${clientRunId}.`,
            );
            // Close the original clientRunId so the UI stops showing the typing indicator.
            // This triggers loadChatHistory in the WebUI, which merges guard session messages.
            broadcastChatFinal({
              context,
              runId: clientRunId,
