...
export async function runEmbeddedPiAgent(
  ...
  // TEMPORARY DIAGNOSTIC (see the [startup-diag] probes below).
  ...
  return enqueueSession(() => {
    ...
    return enqueueGlobal(async () => {
      ...
      // Separates "waited in a queue" from "did slow work": the lane diagnostics
      // report waitMs≈0, so if this is also ≈0 the missing time is in the awaits
      // below rather than in scheduling.
      log.debug(`[startup-diag] queued→body ${started - tEnqueued}ms`);
      ...
      // TEMPORARY DIAGNOSTIC (remove once the startup-latency investigation closes).
      // Gateway logs show ~27s of dead air between a run being enqueued and
      // `embedded run start`, with the event loop blocked (a `node.list` RPC that
      // normally answers in <5ms took 16s inside one such window). These probes
      // attribute that time to a specific await instead of guessing from adjacency —
      // an earlier guess blamed plugin reloading, which turned out to take only 91ms.
      const tPluginsStart = Date.now();
      ensureRuntimePluginsLoaded({
        config: params.config,
        workspaceDir: resolvedWorkspace,
        allowGatewaySubagentBinding: params.allowGatewaySubagentBinding,
      });
      log.debug(`[startup-diag] ensureRuntimePluginsLoaded ${Date.now() - tPluginsStart}ms`);
      ...
      const tModelsJsonStart = Date.now();
      const modelsJsonResult = await ensureOpenClawModelsJson(params.config, agentDir);
      // `wrote` is the load-bearing field: the cache fingerprint includes models.json's
      // own mtime, so a write invalidates the very entry it just populated. Seeing
      // wrote=true on every run would confirm that self-invalidation.
      log.debug(
        `[startup-diag] ensureOpenClawModelsJson ${Date.now() - tModelsJsonStart}ms wrote=${modelsJsonResult.wrote}`,
      );
      ...
      try {
        ...
        while (true) {
          ...
          // TEMPORARY DIAGNOSTIC: closes the bracket on the pre-run gap. `embedded run
          // start` is logged inside the attempt, so anything unaccounted for between
          // `queued→body` and this line lives in the setup awaits above (hook model
          // selection, auth profile init, context engine, models resolution).
          log.debug(`[startup-diag] body→attempt ${Date.now() - started}ms`);
