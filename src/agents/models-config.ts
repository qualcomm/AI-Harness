...
export async function ensureOpenClawModelsJson(
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    await ensureModelsFileModeForModelsJson(targetPath);
    // Re-fingerprint after writing. The fingerprint includes models.json's own
    // mtime, which this write just changed — storing the pre-write value would
    // guarantee a miss on the next call, so the cache could never hit and every
    // run would redo provider discovery.
    const postWriteFingerprint = await buildModelsJsonFingerprint({
      config: cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
    });
    return { fingerprint: postWriteFingerprint, result: { agentDir, wrote: true } };
  });
