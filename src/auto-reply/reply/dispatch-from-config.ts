...
export async function dispatchReplyFromConfig(
  ...
  try {
    ...
    // Run resolve_model hook for channel messages (S3/S2 routing).
    // This mirrors the resolve_model logic in chat.send (gateway/server-methods/chat.ts)
    // so that channel-originated messages (Feishu, Telegram, etc.) also benefit from
    // privacy-level routing — e.g. S3 keywords routed to a local model.
    let channelProviderOverride: string | undefined;
    let channelModelOverride: string | undefined;
    ...
    if (hookRunner?.hasHooks("resolve_model")) {
      try {
        ...
        const hookResult = await hookRunner.runResolveModel(
          {
            ...
            provider: sessionStoreEntry.entry?.providerOverride ?? "",
            model: sessionStoreEntry.entry?.modelOverride ?? "",
            isDefault: !sessionStoreEntry.entry?.modelOverride,
          },
          {
            agentId,
        ...
        if (hookResult?.directResponse) {
          // Plugin handled the LLM call itself — skip agent, send response directly.
          const directPayload: ReplyPayload = { text: hookResult.directResponse };
          if (shouldRouteToOriginating && originatingChannel && originatingTo) {
            await sendPayloadAsync(directPayload);
          } else {
            dispatcher.sendFinalReply(directPayload);
          }
          recordProcessed("completed", { reason: "resolve_model_direct_response" });
          markIdle("message_completed");
          return { queuedFinal: true, counts: dispatcher.getQueuedCounts() };
        }

        if (hookResult?.sessionKey && hookResult.sessionKey !== sessionKey) {
          // S3: redirect to guard/local session.
          ctx.SessionKey = hookResult.sessionKey;
          ctx.ParentSessionKey = sessionKey;
          if (hookResult.userPromptOverride) {
            ctx.BodyForAgent = hookResult.userPromptOverride;
          }
          if (hookResult.extraSystemPrompt) {
            ctx.PrivacySystemPrompt = hookResult.extraSystemPrompt;
          }
        } else if (hookResult?.userPromptOverride) {
          // S2: desensitize content for cloud model.
          ctx.BodyForAgent = hookResult.userPromptOverride;
        }

        if (hookResult?.provider) {
          channelProviderOverride = hookResult.provider;
        }
        if (hookResult?.model) {
          channelModelOverride = hookResult.model;
        }
      } catch (err) {
        logVerbose(`dispatch-from-config: resolve_model hook failed: ${String(err)}`);
      }
    }
    ...
    const replyResult = await replyResolver(
      ctx,
      {
        ...
        providerOverride: channelProviderOverride ?? params.replyOptions?.providerOverride,
        modelOverride: channelModelOverride ?? params.replyOptions?.modelOverride,
