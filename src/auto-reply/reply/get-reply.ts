...
export async function getReplyFromConfig(
  ...
  const hasSessionModelOverride = Boolean(
    ...
    normalizeOptionalString(opts?.modelOverride) ||
    normalizeOptionalString(opts?.providerOverride),
  );
  ...
  if (!useFastTestBootstrap) {
    // Re-assert the gateway-bindable plugin runtime before running hooks. The
    // global hook runner is a process-wide singleton that any loadOpenClawPlugins
    // call can replace (see activatePluginRegistry) — including calls elsewhere
    // in the process that load plugins without allowGatewaySubagentBinding. If
    // one of those runs between gateway startup and this hook firing, a plugin's
    // api.runtime.subagent silently becomes permanently unavailable for the
    // lifetime of that registry. This call is cheap when the active registry is
    // already gateway-bindable (see getCompatibleActivePluginRegistry) — it does
    // not rebuild anything unless the binding was actually lost.
