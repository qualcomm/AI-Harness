...
describe("copyBundledPluginMetadata", () => {
  ...
  it("copies a plugin's prompts directory into dist", () => {
    const repoRoot = makeRepoRoot("openclaw-bundled-plugin-prompts-");
    const pluginDir = createPlugin(repoRoot, {
      id: "dragon-task-agents",
      packageName: "@openclaw/dragon-task-agents",
      packageOpenClaw: { extensions: ["./index.ts"] },
    });
    fs.mkdirSync(path.join(pluginDir, "prompts"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "prompts", "decompose.md"),
      "# Decompose\n",
      "utf8",
    );
  ...
  it("removes stale dist prompts no longer present in source", () => {
    const repoRoot = makeRepoRoot("openclaw-bundled-plugin-prompts-stale-");
    const pluginDir = createPlugin(repoRoot, {
      id: "dragon-task-agents",
      packageName: "@openclaw/dragon-task-agents",
      packageOpenClaw: { extensions: ["./index.ts"] },
    });
    const staleDistPromptsDir = path.join(
      repoRoot,
      "dist",
      "extensions",
      "dragon-task-agents",
    ...
    fs.mkdirSync(staleDistPromptsDir, { recursive: true });
    fs.writeFileSync(path.join(staleDistPromptsDir, "stale.md"), "# stale\n", "utf8");

    copyBundledPluginMetadata({ repoRoot });

    expect(fs.existsSync(staleDistPromptsDir)).toBe(false);
