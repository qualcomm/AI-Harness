...
function copyPluginPromptsDir(params) {
  const sourcePromptsDir = path.join(params.pluginDir, "prompts");
  const targetPromptsDir = path.join(params.distPluginDir, "prompts");
  removePathIfExists(targetPromptsDir);
  if (!fs.existsSync(sourcePromptsDir)) {
    return;
  }
  copySkillPathWithRetry({
    sourcePath: sourcePromptsDir,
    targetPath: targetPromptsDir,
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    copyPluginPromptsDir({ pluginDir, distPluginDir });
