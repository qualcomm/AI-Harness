...
function getPnpmExecPathKind(value) {
  const normalized = path.basename(value).toLowerCase();
  if (/^pnpm(?:-cli)?\.(?:c?js)$/.test(normalized)) {
    return "node-script";
  }
  if (/^pnpm(?:-cli)?\.(?:cmd|exe)$/.test(normalized) || /^pnpm(?:-cli)?$/.test(normalized)) {
    return "native";
  }
  return "unknown";
}
...
export function resolvePnpmRunner(params = {}) {
  ...
  if (typeof npmExecPath === "string" && npmExecPath.length > 0 && isPnpmExecPath(npmExecPath)) {
    const execPathKind = getPnpmExecPathKind(npmExecPath);
    if (execPathKind === "node-script") {
      return {
