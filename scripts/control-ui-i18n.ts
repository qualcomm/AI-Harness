...
async function runProcess(
  ...
  // Both callers run a package manager (`pnpm`, `npm`), which on Windows is a `.cmd`
  // shim. Node refuses to spawn one without a shell, so this fails with ENOENT there
  // unless the shell is enabled — which in turn means arguments are concatenated
  // rather than passed through, hence the quoting.
  const useShell = process.platform === "win32";
  return await new Promise((resolve, reject) => {
    const child = spawn(executable, useShell ? args.map((a) => `"${a}"`) : args, {
