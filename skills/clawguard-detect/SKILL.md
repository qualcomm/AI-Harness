---
name: clawguard-detect
version: 1.0.0
description: Real-time threat detector for OpenClaw runtime environments. Monitors commands, file access, network traffic, and user inputs for attack patterns mapped to MITRE ATT&CK, with ML-based anomaly detection. Use when monitoring active sessions for suspicious activity or evaluating whether a command or input poses a threat.
metadata:
  category: security
  risk: safe
  requires:
