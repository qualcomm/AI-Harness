...
version: 1.0.0
description: Runtime guardian for OpenClaw agent operations. Monitors and intercepts commands, file, and network operations in real time, with session freeze/replay and evidence preservation. Use when starting guardian monitoring, viewing session status or logs, freezing or unfreezing a session, or responding to an emergency.
metadata:
  category: security
  risk: safe
---
