...
version: 1.0.0
description: Security configuration analyzer for OpenClaw instances. Checks configuration files, exposed credentials, file permissions, and network settings, and generates hardening recommendations. Use when reviewing security posture, after configuration changes, or when a user asks for a security check or hardening guidance.
metadata:
  category: security
  risk: safe
---
