...
version: 1.0.0
description: Pre-flight security auditor for OpenClaw Skills. Performs SAST, intent mismatch detection, supply chain verification, and ML-based anomaly detection. Use when installing or auditing a Skill, reviewing an external code source, or when a user asks to check a Skill's security before loading it.
metadata:
  category: security
  risk: safe
---
