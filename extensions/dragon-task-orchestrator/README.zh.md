...
一条流水线是有序的 `{ agentId, instruction }` 步骤列表。每步收到原始请求 + **之前每一步的输出**，每份都标注了步骤号和 agent。这些历史输出共享一份上下文预算并按份数平分，所以步骤再多也不会让 prompt 无界增长。

和 dynamic 模式有两处刻意的差别：

- **某步失败即中断整个运行。** 继续执行会把原始请求交给下一步，而那一步是为了消费前面步骤的输出而写的 —— 产出看起来合理但并不是流水线所描述的东西，这比直接停下更难被发现。
