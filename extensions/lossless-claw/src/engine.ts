...
export class LcmContextEngine implements ContextEngine {
  ...
  constructor(deps: LcmDependencies, database: DatabaseSync) {
    ...
    // Probed inside the success branch below so the init log can report the real
    // value: `this.fts5Available` is not assigned until after this whole block,
    // so reading the field during migration logging yields undefined.
    let fts5AvailableAtInit: boolean | undefined;
    ...
    try {
      ...
      if (tables.length === 0) {
        ...
        migrationOk = true;
        fts5AvailableAtInit = getLcmDbFeatures(this.db).fts5Available;
        this.deps.log.info(
          `[lcm] Migration run completed during engine init: duration=${formatDurationMs(Date.now() - migrationStartedAt)} fts5=${fts5AvailableAtInit}`,
        );
        this.deps.log.debug(
    ...
    this.fts5Available = fts5AvailableAtInit ?? getLcmDbFeatures(this.db).fts5Available;
  ...
  private async interceptInlineImagesInToolMessage(params: {
    ...
    if (
      // `"tool"` is no longer part of the AgentMessage role union, but persisted
      // history written by older versions can still carry it, so the check stays
      // and is compared through a widened view instead of being dropped.
      (params.message.role !== "toolResult" && (params.message.role as string) !== "tool") ||
  ...
  private async interceptLargeToolResults(params: {
    ...
    // Reassigning `params` above discards the `"content" in params.message`
    // narrowing, so the array is read through a widened view — the same approach
    // this function already uses for its top-level tool fields below.
    const messageContent = (params.message as { content?: unknown }).content;
  ...
  async compact(params: {
    ...
    return this.withSessionQueue(
      ...
      async () => {
        ...
        // Budget can also arrive via runtimeContext/legacyParams, which is what
        // resolveTokenBudget exists for. Passing `params.tokenBudget` straight
        // through skipped that fallback and, when absent, reached the core as
        // undefined — where `Math.floor(undefined)` makes the budget NaN. With no
        // budget from any source there is nothing to compact against, so bail out
        // the same way the other precondition checks above do.
        const tokenBudget = this.resolveTokenBudget(params);
        if (tokenBudget === undefined) {
          return {
            ok: true,
            compacted: false,
            reason: "no token budget provided",
          };
        }
  ...
  private async rewriteTranscriptForRotate(params: {
    ...
    const linearizedEntries = entriesToKeep.map((entry) => {
      ...
      // Read `id` from the source entry rather than the spread result: spreading a
      // Record drops its index signature, and `parentId` is the only key the
      // spread overrides, so the two are the same value.
      previousEntryId = typeof entry.id === "string" ? entry.id : previousEntryId;
      return nextEntry;
    });
