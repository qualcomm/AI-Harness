...
function attachRuntimeAuthRequestTransport<TModel extends object>(
  ...
  // The symbol write goes through a non-generic alias: an intersection built on a
  // type parameter is read-only as far as index signatures are concerned.
  const next = { ...model };
  (next as unknown as Record<symbol, unknown>)[
  ...
  for (const profileId of candidates) {
    ...
    if (shouldPreferOAuthHelper) {
      try {
        ...
        // Optional on the pi-ai module surface; when absent there is nothing to
        // refresh and the `refreshed?.apiKey` check below already skips ahead.
    ...
    complete: async ({
      ...
      try {
          ...
          try {
            runtimeAuth = await modelAuth.getRuntimeAuthForModel({
              model: buildModelAuthLookupModel({
                ...
                contextWindow: (resolvedModel as { contextWindow?: number }).contextWindow,
              }),
              cfg: modelAuthConfig,
        ...
        const effectiveReasoning = resolveEffectiveReasoning({
          reasoning,
          reasoningIfSupported,
          modelSupportsReasoning: (resolvedModel as { reasoning?: boolean }).reasoning,
        });
    ...
    resolveSessionTranscriptFile: async ({ sessionId, sessionKey }) => {
      ...
      try {
        ...
        const transcriptPath = api.runtime.agent.session.resolveSessionFilePath(
          ...
          {
            agentId,
            // SessionFilePathOptions takes the sessions DIRECTORY, not the store
            // file path — passing `storePath` here was silently ignored.
