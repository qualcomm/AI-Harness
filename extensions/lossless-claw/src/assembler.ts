...
function extractToolResultIdFromMessage(message: AgentMessage): string | null {
  ...
  // Only some members of the AgentMessage union carry a tool-result id, and the
  // field name differs by provider shape, so probe through a widened view rather
  // than narrowing on a role this function is not given.
  const ids = message as { toolCallId?: unknown; toolUseId?: unknown };
  if (typeof ids.toolCallId === "string" && ids.toolCallId.length > 0) {
    return ids.toolCallId;
  }
  if (typeof ids.toolUseId === "string" && ids.toolUseId.length > 0) {
...
export class ContextAssembler {
  ...
  async assemble(input: AssembleContextInput): Promise<AssembleContextResult> {
    ...
    if (evictableTotalTokens <= remainingBudget) {
      ...
      //
      // Captured into a local because the `hasSearchablePrompt` narrowing above
      // applies to `input.prompt` as a property access, which TS discards inside
      // the callback below.
      const searchPrompt = input.prompt;
      const scored = evictable.map((item, idx) => ({
        item,
        score: scoreRelevance(item.text, searchPrompt),
