...
function synthesizeConversationAnswers(params: {
  ...
  // `BucketExecutionResult`'s failed/skipped branch is one member with
  // `status: "failed" | "skipped"`, not two separate members, so
  // `Extract<BucketExecutionResult, { status: "failed" }>` (member-wise) yields
  // `never` — no member's status is narrowed to exactly "failed". Narrowing to
  // the shared member instead keeps `conversationId`/`error` typed on both.
  type FailedOrSkippedResult = Extract<BucketExecutionResult, { status: "failed" | "skipped" }>;
  const failedResults = params.results.filter(
    (result): result is FailedOrSkippedResult => result.status === "failed",
  );
  const skippedResults = params.results.filter(
    (result): result is FailedOrSkippedResult => result.status === "skipped",
  );
