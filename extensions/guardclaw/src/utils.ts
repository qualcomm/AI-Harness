...
/**
 * Check if a path refers to protected session transcript files.
 *
 * Two protection modes:
 *
 * 1. No `targetAgentId` (cloud model check):
 *    Blocks access to ANY agent's session transcripts (.jsonl).
 *    Cloud models must never read raw conversation history.
 *
 * 2. With `targetAgentId` (cross-agent isolation check):
 *    Blocks access to a SPECIFIC agent's session transcripts.
 *    Used to prevent the guard agent from reading the main agent's session history —
 *    guard agent sessions are isolated and must not accumulate main agent context.
 *
 * Note: openclaw does not currently enforce cross-agent session isolation at the
 * framework level, so this hook-level guard is the primary enforcement mechanism.
 */
export function isProtectedMemoryPath(
  filePath: string,
  baseDir: string = "~/.openclaw",
  ...
  if (targetAgentId) {
    // Protect a specific agent's session transcripts (cross-agent isolation)
    // Path pattern: <baseDir>/agents/<targetAgentId>/sessions/**/*.jsonl
    const agentSessionsDir = `${normalizedBase}/agents/${targetAgentId}/sessions/`;
    return normalizedFile.startsWith(agentSessionsDir) && normalizedFile.endsWith(".jsonl");
  }

  // Protect any session transcript from cloud model access
  // Path pattern: <baseDir>/agents/*/sessions/**/*.jsonl
  const sessionsDirPattern = `${normalizedBase}/agents/`;
  return (
    normalizedFile.startsWith(sessionsDirPattern) &&
    normalizedFile.includes("/sessions/") &&
    normalizedFile.endsWith(".jsonl")
  );
}
