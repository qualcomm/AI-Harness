...
function mergeDetectionResults(results: DetectionResult[]): DetectionResult {
  if (results.length === 0) {
    // Fail-closed: if all detectors failed (threw exceptions), treat as S3.
    // An empty result set means the privacy protection layer is completely
    // unavailable — passing content through as S1 would silently bypass all
    // privacy controls. S3 ensures the request is blocked or routed locally.
    return {
      level: "S3",
      levelNumeric: 3,
      reason: "All detectors failed — failing closed to prevent data leakage",
      detectorType: "ruleDetector",
      confidence: 0,
    };
  }
